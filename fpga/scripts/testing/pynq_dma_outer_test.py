...
def test_pynq_dma(test_id: int, num_neurons: int, num_synapses: int, num_inputs: int, num_outputs: int, num_sim_times: int, sim_time: int, zynq_host: str, zynq_serial_device: str, zynq_password: str) -> float:
    ...
    with open(str(proc_tool_commands_fpath), 'w') as f:
        for line in proc_tool_commands_lines:
            if "ml" not in line.lower():
                ...
                f.write("ML /home/petalinux/zynq_framework/cpp-apps/networks/" + str(TEST_DIR_REL_PATH_BASE) + str(test_id) + "_" + rand_net_json_fpath.name + "\n")
    ...
    if "are identical" in serial_output:
        test_time_match = re.search("real\s*(\d*)h?(\d+)m(\d+\.\d+)s", serial_output)
        if test_time_match:
            test_times = test_time_match.groups()
            if len(test_times) >= 3:
                hours = 0
                if test_times[0] != "":
                    hours = float(test_times[0])
                ret = hours*3600.0 + float(test_times[1])*60.0 + float(test_times[2])
    
    return ret


if __name__ == '__main__':
    res = test_pynq_dma(0, 10, 12, 4, 4, 2, 30, "10.42.0.128", "/dev/ttyUSB1", "tennlab")
    print(res)
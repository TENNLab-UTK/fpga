...
parser = argparse.ArgumentParser(
    prog="uart_loop", description="UART Loopback Baud Rate Test"
)
parser.add_argument("target", type=str, help="Target device name (e.g. 'basys3')")
parser.add_argument("dev", type=pl.Path, help="cdev path for UART (e.g. '/dev/ttyS1')")
parser.add_argument(
    "-n",
    dest="num_bytes",
    type=int,
...
config_fname = resources.files(config).joinpath("targets.json")
...
rtl_path = pl.Path(
    os.path.relpath(
        (pl.Path(resources.files(rtl))).resolve(),
        start=proj_path.resolve(),
    )
)
config_path = pl.Path(
    os.path.relpath(
        (pl.Path(resources.files(config))).resolve(),
        start=proj_path.resolve(),
    )
)
